...
sudo mkdir -p /var/log/autoplius-scraper
sudo touch "$TRANSLATE_LOG"
sudo chown autoplius:autoplius "$TRANSLATE_LOG" || true
nohup sudo -u autoplius env PYTHONPATH="$APP" "$PY" \
  backfill_descriptions_ru.py --ids 32156004,32155944,32155948,32155970,32155940,32093378,32064212 \
  >>"$TRANSLATE_LOG" 2>&1 &
nohup sudo -u autoplius env PYTHONPATH="$APP" "$PY" \
  backfill_descriptions_ru.py --active-only --limit 250 \
  >>"$TRANSLATE_LOG" 2>&1 &
